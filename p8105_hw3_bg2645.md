...
```
  ...
  - The key variables in the `instacart` dataset were the porducts that
    ...
    and the unique user\_id for each customer. \*Thusased on the
...
```
  ...
  - The key variables in the `accel_data` dataset was the week, day id,
    ...
    whether the day was a weekday or not.
...
![](p8105_hw3_bg2645_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->
  ...
  - Based on the plot above, total activity time decreases on weekends
    ...
    constant across days. There was drastic spike in actvity levels on
    Monday on day 10 to ~day 16 and there was also another drastic spike
    in activity level on Friday from day 22 to 28.
